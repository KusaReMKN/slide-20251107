...
\begin{frame}
  ...
  \note{
    ...
    信濃眞伊が発案したものを上羽未栞が実装しようと試みて、
    その途中で佐伯真紘が命名したよ。
    実装にあたっては一式すみれによる監修があったよ。
...
\begin{frame}
  ...
  \hspace{1.5\zw}PPPのマネをすればユーザの世界から通信を吹き込める

  TUN/TAP（トンネルデバイス）を用いることで\\
  \hspace{1.5\zw}「普通のプログラム」からでも通信を実現できる
  ...
  \note{
    ...
    これはファイルのように読み書きできるよ。
    PPPのマネをすればユーザの世界から通信を吹き込めるということなので、
    これについて調べてみると、
    TUN/TAP（トンネルデバイス）というものに辿り着くよ。
...
\begin{frame}
  ...
  通信実験ではパケットロス率XX\%\\
  \hspace{1.5\zw}平均往復時間XX msを記録した
